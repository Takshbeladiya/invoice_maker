...
def generate_excel_report(blinds_data, session_state, active_products):
    ...
    - Adds Total, Split, Resize, and Mount columns.
    ...
    headers = ['ID', 'Desc', 'W', 'H', 'Undivided', 'Total', 'Split', 'Resize', 'Mount', 'Total Sq Ft']
    ...
    for blind in sorted(blinds_data, key=lambda x: x['id']):
        ...
        ws.cell(row=row_num, column=8, value="Yes" if blind.get('resize_width') else "No") # Resize
        ws.cell(row=row_num, column=9, value=blind['mount']) # Mount
        ...
        # Formula for Total Sq Ft (Col J) - uses undivided blinds for area calculation
        sqft_formula = f"=IFERROR((({w_cell}*{h_cell})/144)*{num_cell_undivided}, 0)"
        ws.cell(row=row_num, column=10, value=sqft_formula)
        sqft_cell = f'J{row_num}'
        
        # Formulas for each product cost
        product_cost_cells = []
        col_idx = 11 # Start of product cost columns
        ...
        # Note: Simplified formula uses original width for all pieces for Excel clarity
    ...
    if session_state.motor_quantity > 0:
        ...
        first_product_col_idx = 11
        shipping_col_idx = 10 + len(active_products) + 1
    ...
    shipping_col_letter = get_column_letter(10 + len(active_products) + 1)
    ...
    for i, p in enumerate(active_products):
        product_col_letter = get_column_letter(11 + i)
